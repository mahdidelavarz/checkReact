...
function ThreeStepRecord({ step, footerNextFunc, footerAgainFunc, status }) {
  return (
    <div className="flex-1 flex flex-col justify-evenly">
      {status ? (
        <CustomText className="font-bold text-center mt-1 text-gray-800 flex items-center justify-center">
          <img className="w-4 h-4 mr-1" src={icOk} alt="OK Icon" />{" "}
          {languages("send_done")}
        </CustomText>
      ) : (
        <CustomText className="font-bold text-center mt-1 text-gray-800 flex items-center justify-center">
          <img
            className="w-4 h-4 mr-1 filter hue-rotate-0 brightness-75"
            src={icClose}
            alt="Close Icon"
          />{" "}
          ارسال فیلد دوم با مشکل مواجه شد دوباره تلاش کنید
        </CustomText>
      )}
